...
class ClearanceAPIServer:
    ...
    def __init__(self, headless: bool, thread: int, page_count: int,
        ...
        self.app = FastAPI(title="global-captcha", version="1.3.1")
        ...
        self.app.post("/solve")(self.process_llm_solve)
    ...
    def _load_proxies(self):
        ...
        if not os.path.isfile(self.proxy_file):
            logger.warning(f"proxy_support enabled but file '{self.proxy_file}' not found.")
            return
    ...
    async def _cleanup_results(self):
        """Remove stale results (success/error) older than 10 minutes.
        Still running tasks 'process' are NOT removed — just marked as timeout
        if already past 300 detik, so the client gets a proper response.
        """
        while True:
            ...
            for tid, res in list(self.results.items()):
                ...
                if status == "process":
                    # If task has been stuck in process, tandai error
                    # so clients do not wait forever — tapi JANGAN hapus dulu
                    if age > 300:
                        to_timeout.append(tid)
                else:
                    # Hapus hasil success/error older than 10 minutes
                    if age > 600:
            ...
            for tid in to_timeout:
                if self.results.get(tid, {}).get("status") == "process":
                    ...
                    self.results[tid] = {
                        ...
                        "value": "timeout",
                        "message": "Task timeout: did not complete dalam 300 detik",
                        "start_time": start_t,
                    }
            if to_timeout:
    ...
    async def _periodic_cleanup(self, interval_minutes: int = 10):
        """
        Periodic cleanup PAKSA yang memfreekan RAM:
        - Runs every interval_minutes (configurable in config.json)
        - Does not wait for all workers to finish — force cleanup immediately
        - Wait for pages in use (max 60 detik), lalu full restart
        """
        DRAIN_TIMEOUT = 60  # Max wait time untuk pages in use (detik)

        while True:
            await asyncio.sleep(interval_minutes * 60)
            logger.info(f"[Cleanup] Starting FORCED cleanup (interval: {interval_minutes} menit)...")
            ...
            if busy_count > 0:
                # Pages in use, tunggu mereka selesai (max DRAIN_TIMEOUT)
                logger.info(f"[Cleanup] Waiting for {busy_count} pages in use (max {DRAIN_TIMEOUT}s)...")
                ...
                if still_busy > 0:
                    logger.warning(f"[Cleanup] {still_busy} page still in use after timeout, force restart without them")
            ...
            # Fase 3: Tutup semua context lama (ini yang freekan RAM Firefox)
            ...
            try:
                ...
                pass
            if stale:
                logger.info(f"[Cleanup] Closing {len(stale)} stale pages from pool")
            ...
            try:
                await self._build_page_pool()
                logger.success(f"[Cleanup] Full context restart selesai, {self.page_pool.qsize()} pages ready")
            except Exception as e:
                logger.error(f"[Cleanup] Failed to rebuild pool: {e}, mencoba restart browser...")
                try:
                    try:
                        await self.browser.close()
                    except Exception:
                    ...
                    self.browser = await self.camoufox.start()
                    await self._build_page_pool()
                    logger.success(f"[Cleanup] Browser restart successful, {self.page_pool.qsize()} pages ready")
                except Exception as e2:
                    logger.error(f"[Cleanup] FATAL: Failed to restart browser: {e2}")
    ...
    async def _save_debug_on_fail(self, page, task_id: str, round_num: int, url: str):
        """ Save screenshot + HTML + page info to debug_logs/ on failure."""
        try:
            ...
            logger.info(f"[Debug] Saved: {prefix}.png + .json")
        except Exception as e:
            logger.warning(f"[Debug] Failed to save debug: {e}")
    ...
    async def _solve_turnstile(self, task_id: str, url: str, sitekey: str,
        ...
        try:
            ...
            for round_num in range(1, MAX_ROUNDS + 1):
                if round_num > 1:
                    logger.info(f"[Turnstile] Round {round_num}: reload halaman — {task_id}")
                    if self.proxy_support and self.proxies:
                        ...
                        try:
                            ...
                            logger.info(f"[Turnstile] Proxy rotated: {proxy} — {task_id}")
                        except Exception as e:
                            logger.warning(f"[Turnstile] Failed to rotate proxy: {e}, keeping old context")
                ...
                for attempt in range(80):  # 80 × 0.3s = ~24 detik timeout
                    try:
                        ...
                        logger.debug(f"[Turnstile] Round {round_num} attempt {attempt + 1} failed: {e}")

                if not solved:
                    logger.warning(f"[Turnstile] Round {round_num} failed 30x — {task_id}, {'retry...' if round_num < MAX_ROUNDS else 'giving up'}")
                    # Save debug info on each failed round
            ...
            logger.warning(f"[Turnstile] Giving up after {MAX_ROUNDS} putaran — {task_id}")
        except Exception as e:
            elapsed = round(time.time() - start_time, 3)
            self.results[task_id] = {"status": "error", "elapsed_time": elapsed, "value": "captcha_fail"}
            ...
            try:
                ...
                pass
            # Only return to pool if context still valid (not restarted by cleanup)
            if context in self.contexts:
                await self.page_pool.put((page, context))
            else:
                logger.info(f"[Turnstile] Page discarded (context was restarted oleh cleanup) — {task_id}")
    ...
    async def _solve_clearance(self, task_id: str, url: str, timeout: int = 30):
        ...
        try:
            ...
            logger.info(f"[Clearance] Navigating to {url} — {task_id}")
            ...
            if cf_clearance:
                # Collect all cookies from domain
                ...
                self.results[task_id] = {
                    ...
                    "message": f"cf_clearance not found setelah {timeout}s. Page title: '{title}'",
                }
            ...
            # Reset page & clear cookies agar tidak membawa state lama
            try:
            ...
            if context in self.contexts:
                ...
                logger.info(f"[Clearance] Page discarded (context was restarted oleh cleanup) — {task_id}")
    ...
    async def _solve_aws_token(self, task_id: str, url: str, timeout: int = 30):
        ...
        Jika IP diblok CloudFront, otomatis retry dengan proxy (jika available).
        """
        start_time = time.time()
        page, context = await self.page_pool.get()
        _temp_context = None  # temporary context for proxy retry
        ...
        async def _navigate_and_poll(p, ctx, attempt_label: str):
            ...
            while time.time() < deadline_poll:
                ...
                if waf_cookie:
                    logger.debug(f"[AWS-Token][{attempt_label}] ✅ aws-waf-token found at poll #{poll_count}")
                    break
        ...
        try:
            logger.info(f"[AWS-Token] Navigating to {url} — {task_id}")
            ...
            if was_blocked or (not waf_cookie):
                if was_blocked:
                    logger.warning(f"[AWS-Token] VPS IP blocked by CloudFront. Trying proxy... ({len(self.proxies)} available)")
                    ...
                    try:
                        ...
                        try:
                            ...
                            if w2:
                                ...
                                logger.info(f"[AWS-Token] ✅ Proxy bypassed CloudFront block")
                            else:
                                logger.warning(f"[AWS-Token] Proxy also failed. Judul: '{t2}' | blocked: {b2}")
                        finally:
                            try:
                        ...
                        logger.error(f"[AWS-Token] Failed to create proxy context: {proxy_err}")
                        logger.debug(f"[AWS-Token] Proxy error detail: {type(proxy_err).__name__}: {proxy_err}")
                else:
                    if was_blocked:
                        logger.error(
                            "[AWS-Token] 🚫 VPS IP blocked by CloudFront dan tidak ada proxy available!\n"
                            "         → Enable proxy_support=true di config.json dan isi proxies.txt"
                        )
            ...
            if waf_cookie:
                ...
                try:
                    ...
                    logger.debug(f"[AWS-Token] Cookies on failure: {cookie_dump}")
                    ...
                    try:
                        ...
                        logger.debug(f"[AWS-Token] Screenshot failed: {ss_err}")
                    ...
                    self.results[task_id] = {
                        ...
                        "value": "waf_fail",
                        "message": (
                            f"aws-waf-token not found setelah {elapsed}s "
                            f"({polls} polls). HTTP {http_status}. "
                            f"Judul: '{fail_title}'{block_note}"
                        ),
                    }
                except Exception as diag_err:
                    logger.debug(f"[AWS-Token] Diagnosis failed: {diag_err}")
                    self.results[task_id] = {
                        "status": "error",
                        "elapsed_time": elapsed,
                        "value": "waf_fail",
                        "message": f"aws-waf-token not found setelah {timeout}s",
                    }
            ...
            # Close temporary proxy context if any
            ...
            if context in self.contexts:
                ...
                logger.info(f"[AWS-Token] Page discarded (context was restarted) — {task_id}")
    ...
    async def _solve_recaptcha(self, task_id: str, url: str, sitekey: str, action: str):
        ...
        try:
            ...
            logger.info(f"[reCAPTCHA] Setting up Chromium browser — {task_id}")
            ...
            logger.info(f"[reCAPTCHA] Opening URL: {url} — {task_id}")
            ...
            if not has_recaptcha:
                logger.info(f"[reCAPTCHA] Script not detected, menyuntikkan script api.js secara manual — {task_id}")
                inject_js = """(key) => {
                    return new Promise((res, rej) => {
                        ...
                        s.onerror = () => rej(new Error("Failed to load script Google reCAPTCHA"));
                        document.head.appendChild(s);
                    });
                }"""
                await page.evaluate(inject_js, sitekey)
                logger.info(f"[reCAPTCHA] Script injected manually — {task_id}")
            else:
                logger.info(f"[reCAPTCHA] Script already detected on page — {task_id}")

            logger.info(f"[reCAPTCHA] Executing grecaptcha.execute (Action: '{action}') — {task_id}")
            solve_js = """([key, act]) => {
                return new Promise((res, rej) => {
                    grecaptcha.ready(() => {
                        grecaptcha.execute(key, { action: act })
                            .then(res)
                            .catch(err => rej(new Error(err.message || "Failed to get token")));
                    });
                });
            }"""
            ...
            logger.success(f"[reCAPTCHA] Successfully solved — {task_id} ({elapsed}s)")
            ...
            self.results[task_id] = {
                ...
                "elapsed_time": elapsed,
                "value": "captcha_fail",
                "message": f"Failed to solve reCAPTCHA: {str(e)}"
            }
    ...
    async def process_turnstile(self, url: str = Query(...), sitekey: str = Query(...),
        ...
        if not url or not sitekey:
            raise HTTPException(status_code=400, detail={"status": "error", "error": "Parameter 'url' dan 'sitekey' is required"})

        if self.page_pool.qsize() == 0:
            return JSONResponse(content={"status": "error", "error": "Server full, try again nanti"}, status_code=429)
    ...
    async def process_clearance(
        ...
        - user_agent   : User-Agent that MUST be used with this cookie
        - cookies      : seluruh cookie domain dalam format header (opsional, untuk kemudahan)
        """
        if not url:
            raise HTTPException(status_code=400, detail={"status": "error", "error": "Parameter 'url' is required"})
    ...
    async def get_result(self, task_id: str = Query(..., alias="id")):
        if not task_id:
            return JSONResponse(content={"status": "error", "message": "Parameter id is required"}, status_code=400)
        if task_id not in self.results:
            return JSONResponse(content={"status": "error", "message": "task_id invalid or expired"}, status_code=404)
    ...
    async def process_llm_solve(self, request: Request):
        """
        Solve an image-based CAPTCHA via the embedded LLM solver.

        Body (JSON):
            image       : str  - base64-encoded image, data URL, or http(s) URL
            type        : str  - captcha type: image_text, math, hcaptcha_image,
                                 recaptcha_v2, object_select
            hint        : str  - optional hint (e.g. "select all bicycles")

            # Backend selection
            use_hermes_cli : bool - default True. Use `hermes chat -q --image`
                                       (slower, inherits hermes config).
                                 Set to False to use the HTTP router chain
                                 (faster, requires env vars or overrides below).

            # Optional LLM backend override for HTTP router mode (use_hermes_cli=False):
            router_url  : str  - full /v1/chat/completions URL
            api_key     : str  - bearer token
            model       : str  - primary model name
            fallback    : str | list[str] - additional fallback model(s)
            max_tokens  : int  - override solve.max_tokens
            temperature : num  - override solve.temperature

            When any of the override fields above is omitted, falls back to
            env vars / config.local.yaml (started with the server).

        Returns the same JSON shape as the standalone solver:
            {answer, raw, model, elapsed_s, tokens, type, source}
        """
        try:
            body = await request.json()
        except Exception as e:
            raise HTTPException(status_code=400, detail={"error": f"invalid JSON body: {e}"})

        image = body.get("image", "")
        ctype = body.get("type", "image_text")
        hint = body.get("hint", "")
        use_hermes_cli = body.get("use_hermes_cli", True)  # NEW DEFAULT

        # Collect optional LLM override fields (only if non-empty, for HTTP router mode)
        llm_override = {}
        for k in ("router_url", "api_key", "model", "fallback", "max_tokens", "temperature"):
            v = body.get(k)
            if v not in (None, ""):
                llm_override[k] = v
        if not llm_override:
        ...
        if not image:
            raise HTTPException(status_code=400, detail={"error": "'image' field required"})

        # lazy-import solver module (avoids config validation at server startup)
        try:
            import importlib.util
            from pathlib import Path
            _here = Path(__file__).parent
            _solver_path = _here / "solver.py"
            if not _solver_path.exists():
                raise FileNotFoundError(
                    f"solver.py not found at {_solver_path}. "
                    "Install it or set CAPTCHA_LLM_SOLVER_PATH env var."
                )
            _spec = importlib.util.spec_from_file_location("captcha_llm_solver", _solver_path)
            _mod = importlib.util.module_from_spec(_spec)
            _spec.loader.exec_module(_mod)
        except SystemExit as e:
            raise HTTPException(
                status_code=503,
                detail={"error": f"LLM solver config invalid: {e.code if hasattr(e, 'code') else e}"},
            )
        except Exception as e:
            raise HTTPException(status_code=500, detail={"error": f"failed to load LLM solver: {e}"})

        try:
            result = _mod.solve_with_fallback(
                image=image,
                captcha_type=ctype,
                hint=hint,
                llm_override=llm_override,
                use_hermes_cli=use_hermes_cli,
            )
            # RouterSolveResult is a pydantic model — convert to dict for JSONResponse
            if hasattr(result, "model_dump"):
                payload = result.model_dump()
            elif hasattr(result, "dict"):
                payload = result.dict()
            else:
                payload = dict(result) if isinstance(result, dict) else {"answer": str(result)}
            return JSONResponse(content=payload)
        except SystemExit as e:
            raise HTTPException(
                status_code=503,
                detail={"error": f"LLM solver failed: {e.code if hasattr(e, 'code') else e}"},
            )
        except Exception as e:
            logger.exception("llm-solve failed")
...
def _auto_install():
    ...
    if not _camoufox_data_exists():
        print("  🌐  Downloading Camoufox browser data...")
        subprocess.check_call([sys.executable, "-m", "camoufox", "fetch"])
        print("  ✅  Camoufox downloaded")
    print("═" * 52 + "\n")
...
CONFIG_PATH = os.environ.get(
    ...
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "captcha_solver_config.json"),
)
CONFIG_DEFAULTS = {
    "headless":      True,
    "thread":        2,
...
def _load_config() -> dict:
    try:
        ...
        print(f"  ⚠️  {CONFIG_PATH} not found, menggunakan nilai default\n")
        return dict(CONFIG_DEFAULTS)
    except json.JSONDecodeError as e:
        print(f"  ❌  Format {CONFIG_PATH} invalid: {e}\n")
        return dict(CONFIG_DEFAULTS)
...
def _show_config_summary(cfg: dict):
    ...
    labels = {
        ...
        "thread":        ("Browser instances",          "int"),
        "page_count":    ("Pages per instance",             "int"),
        "proxy_support": ("Proxy Support",                   "bool"),
        "host":          ("Server Host",                      "str"),
        "port":          ("Server Port",                      "int"),
        "debug":         ("Mode Debug",                       "bool"),
        "cleanup_interval_minutes": ("Forced cleanup interval (min)", "int"),
    }
...
def _interactive_config(cfg: dict) -> dict:
    ...
    print()
    ans = input("  ▶  Continue? [Enter/Y = ya  |  N = ubah] : ").strip().lower()
    ...
    labels = {
        ...
        "page_count":    "Pages per instance",
        "proxy_support": "Proxy Support (true/false)",
        "host":          "Server Host",
        "port":          "Server Port",
        "debug":         "Mode Debug (true/false)",
        "cleanup_interval_minutes": "Forced cleanup interval (min)",
    }
    print("\n  ✏️   Press Enter to keep current value")
    print("─" * 52)
    new_cfg = dict(cfg)
    ...
    if input("\n  💾  Save? [Y/Enter = ya  |  N = tidak] : ").strip().lower() not in ("n", "no"):
...
def _check_port(cfg: dict) -> dict:
    ...
    while True:
        ...
        try:
            new_port = int(input(f"  🔁  Replacement port : ").strip())
            cfg = dict(cfg)
            cfg["port"] = new_port
            if input(f"  💾  Save port {new_port}? [Y/Enter] : ").strip().lower() not in ("n", "no"):
                _save_config(cfg)
        except ValueError:
...
def _check_system(cfg: dict):
    ...
    print("  💻  SYSTEM INFO")
    print("═" * 52)
    print(f"  🖥️  CPU  : {cpu_count} core | used {cpu_usage:.1f}%")
    print(f"  🧠  RAM  : {ram_total:.1f} GB | free {ram_free:.1f} GB ({ram_pct:.1f}%)")
    ...
    if est_ram > ram_free * 0.85:
        issues.append(f"  ⚠️  Est. RAM ({est_ram:.1f} GB) mendekati RAM free ({ram_free:.1f} GB)")
    if cpu_usage > 80:
    ...
    if issues:
        ...
        print("  ✅  Config matches system resources")
    print("═" * 52 + "\n")
...
# ──────────────────────────────────────────────
def _print_banner():
    """Print the captcha banner."""
    banner = """
 ═══════════════════════════════════════════════
 │ CAPTCHA Solver (Modelgate)        │
 │ Version: 1.0                                 │
 │ Powered by Camoufox + FastAPI                │
 ═══════════════════════════════════════════════
    """
    print(banner)
...
def _run_server():
 ...
 ═══════════════════════════════════════════════
    ...
    print()
    print("═" * 52)
    print("  🔍  CHECK PORT & SYSTEM RESOURCES")
    print("═" * 52)
